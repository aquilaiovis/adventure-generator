    ...
    public void handlePacket(SocketAddress address, Packet packet)
    {
        ...
        else if (packet instanceof PacketPosition)
        {
            ...
            for (Player p : world.getPlayers())
            {
                ...
                {
                    p.setPosition(new Point(pp.getPositionX(), pp.getPositionY(), pp.getPositionZ()));
                    p.setPerspective(new Point(pp.getPerspectiveX(), pp.getPerspectiveY(), pp.getPerspectiveZ()));
                }
            }
        }
